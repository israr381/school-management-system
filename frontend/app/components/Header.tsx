...
export default function Header({ user, isSuperAdmin, onLogout, org }: HeaderProps) {
  ...
  return (
    <header className="flex h-17 shrink-0 items-center gap-4 rounded-xl border border-header-border bg-header-bg px-4 shadow-header lg:px-5">
      ...
      <div className="hidden w-full max-w-135 min-w-0 md:block">
        <div className="relative w-full">
          ...
          <input
            ...
            className="h-11 w-full rounded-md border border-header-search-border bg-header-search-bg pl-11 pr-24 text-sm text-text-main placeholder:text-text-muted/70 outline-none transition-colors focus:border-brand/40 focus:ring-2 focus:ring-brand/20"
          />
          <kbd className="pointer-events-none absolute right-2.5 top-1/2 hidden -translate-y-1/2 items-center rounded-lg border border-header-kbd-border bg-header-kbd-bg px-2 py-1 text-[10px] font-medium tracking-wide text-text-muted sm:inline-flex">
            Ctrl + K
          </kbd>
        </div>
      </div>
