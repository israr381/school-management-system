...
import { getHeroQuickStats, type TenantApiResponse } from "./systemData";
...
const quickStatIcons = [Building2, Globe, Server, ShieldCheck];
...
export default function SuperAdminHero({
  ...
  const quickStats = getHeroQuickStats(tenantData, statsLoading);
    ...
    <section className="dashboard-hero relative overflow-hidden rounded-[20px] px-6 py-7 sm:px-8 sm:py-8 lg:px-10">
      ...
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex-1">
          <h1 className="mt-3 text-2xl font-bold tracking-tight text-white sm:text-[28px] lg:text-[32px]">
            Welcome back, {userName} 👋
          </h1>
          <p className="mt-2 text-sm text-white/80 sm:text-base">
            Here&apos;s your system-wide overview across all organizations on the platform.
          </p>

          <div className="mt-6 flex flex-wrap gap-2.5 sm:gap-3">
            {quickStats.map((stat, idx) => {
              const Icon = quickStatIcons[idx];
              return (
                <div
                  key={stat.label}
