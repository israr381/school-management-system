  ...
  return (
    ...
    >
      <SelectTrigger
        size="sm"
        disabled={disabled}
        className="h-7 min-w-22 rounded-md border-border-main bg-input-bg px-2 text-xs font-medium text-text-main shadow-none dark:bg-input-bg dark:hover:bg-input-bg"
      >
      ...
      <SelectContent
        ...
        className="max-h-60 min-w-28 rounded-md border-border-main bg-panel-bg text-text-main"
      >
        {(options ?? []).map((option) => (
          <SelectItem
            key={option.value}
    ...
    <DayPicker
      ...
      classNames={{
        ...
        caption_label: cn(
          ...
          captionLayout === "label"
            ? "text-sm"
            : "pointer-events-none flex items-center gap-1 rounded-(--cell-radius) px-1.5 py-0.5 text-sm [&>svg]:size-3.5 [&>svg]:text-muted-foreground",
